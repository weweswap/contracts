...
export async function main(owner: string, asset?: string) {
  ...
  const MintNftPositionFactory =
    await ethers.getContractFactory('MintNftPosition');
  const mintNftPosition = await MintNftPositionFactory.deploy(
    asset || WEWE_ADDRESS,
    poolAddress,
  ...
  const weweContract = await ethers.getContractAt(
    'IERC20',
    asset || WEWE_ADDRESS,
    deployer,
  );
  ...
  await wethContract.approve(
    await mintNftPosition.getAddress(),
    amountToDeposit0,
  );
  await weweContract.approve(
    await mintNftPosition.getAddress(),
    amountToDeposit1,
  );

  const tx = await mintNftPosition
    .connect(deployer)
    .mintNewPosition(amountToDeposit0, amountToDeposit1, owner);
  await tx.wait();
}
