...
describe("Dynamic Merge Contract", function () {
	...
	async function deployFixture() {
		...
		const token = await Token.deploy("Token", "TKN", 9);

		// Max supply of 1000 tokens to eat
		const vestingPeriod = 60;
		const virtualFomo = ethers.parseUnits("800", 9);
		const virtualWeWe = ethers.parseUnits("1000", 18);
	...
	describe("Merge with dynamic rates", () => {
		...
		it.only("Should calcuate rates with large numbers", async () => {
			...
			// In 10^18 we we
			let weweAmount = await merge.calculateTokensOut(ethers.parseUnits("1", 9));
