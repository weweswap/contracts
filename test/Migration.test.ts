...
describe("Migration contract", function () {
	...
	describe("Configuration", function () {
		...
		it("Should revert if deployed with a zero address", async function () {
			const Migration = await ethers.getContractFactory("Migration");
			await expect(Migration.deploy(ethers.ZeroAddress, SWAP_ROUTER_ADDRESS, WEWE_ADDRESS, USDC_ADDRESS, 3000)).to.be.revertedWith(
				"Migration: Invalid NonfungiblePositionManager address",
			);
			await expect(Migration.deploy(UNI_V3_POS, ethers.ZeroAddress, WEWE_ADDRESS, USDC_ADDRESS, 3000)).to.be.revertedWith(
