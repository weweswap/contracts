...
describe.only("UniswapV3 Adaptor", () => {
	...
	describe("Uni adaptors", () => {
        ...
        it.skip("Should call uniswap with dynamic via the adaptor", async () => {
            const { uniswapAdaptor, holder } = await deployFixture("UniswapDynamicFee");
            ...
            const fee = "0x2710";
            // Swap 100 of the 5800 usdc for wewe via uniswap on fork at block 20820713
            await uniswapAdaptor.connect(holder).swap(ethers.parseUnits("100", 6), USDC_ADDRESS, fee);
		...
		it.only("Should merge Token and swap via uni to wewe", async () => {
            ...
            // const adaptor_balance_after = await token.balanceOf(uniswapAdaptorAddress);
            // expect(adaptor_balance_after).to.be.gt(0);
		});
	});
});
