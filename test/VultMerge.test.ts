...
describe.only("Vult Merge Contract", function () {
	async function deployFixture() {
		...
		const Vult = await ethers.getContractFactory("VultMerge");
	...
	describe("receiveApproval Function", function () {
		it("Should revert if called by a non-WEWE token", async function () {
			...
			await expect(mockWewe.approveAndCall(merge, 1000, "0x")).to.be.revertedWithCustomError(merge, "InvalidTokenReceived");
		});
	...
	describe("onApprovalReceived Function", function () {
		it("Should revert if called by a non-VULT token", async function () {
			...
			await expect(mockVult.approveAndCall(merge, 100, "0x")).to.be.revertedWithCustomError(merge, "InvalidTokenReceived");
		});
	...
	describe("Deposit Function", function () {
		it("Should revert if token is not WEWE or VULT, not called from non-owner", async function () {
			...
			await expect(merge.connect(otherAccount).deposit(vult, 100)).to.be.revertedWith("Ownable: caller is not the owner");
		});
	...
	describe("Setters", function () {
		it("Should correctly set LockedStatus", async function () {
			...
			await expect(merge.connect(otherAccount).setLockedStatus(1)).to.be.revertedWith("Ownable: caller is not the owner");
		});
		...
		it("Should correctly set VirtualWeweBalance", async function () {
			...
			// Called from non-owner failed
			await expect(merge.connect(otherAccount).setVirtualWeweBalance(virtualBalance)).to.be.revertedWith("Ownable: caller is not the owner");
		});
	});
});
